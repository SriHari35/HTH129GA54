...
app.get("/user", (req, res) => {
    ...
    async function analyzeWithBackend(code, language) {
    try {
        ...
        console.log("CodeGuard AI response:", result);

        return result;

    } catch (error) {
        console.error("CodeGuard backend error:", error);

        alert(
            "Unable to connect to CodeGuard AI backend.\n\n" +
...
async function sendCodeToBackend(code, language) {
    try {
        const response = await fetch("http://localhost:3000/api/review", {
            method: "POST",
            headers: {
        ...
        console.log("CodeGuard AI:", result);
        ...
        alert(
            "CodeGuard AI backend is not reachable.\n\n" +
...
const response = await client.responses.create({
    model: "gpt-5.6-luna",
    input: `Review this ${language} code for security, bugs,
